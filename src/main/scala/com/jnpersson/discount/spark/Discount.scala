...
final case class Discount(k: Int, minimizers: MinimizerSource = Bundled, m: Int = 10,
  ...
  protected def newSession(buckets: Int): SparkSession = {
  ...
  /** Construct an index with synthetic (random) reads.
   * @param n The number of reads to generate.
   */
  def syntheticIndex(n: Long): Index =
    new Kmers(this, Seq.empty, None, Some(n))(newSession(partitions)).index
...
class Kmers(val discount: Discount, val inFiles: Seq[String], knownSplitter: Option[Broadcast[AnyMinSplitter]] = None,
  ...
  protected def inputSequences =
    syntheticReads match {
      case Some(numReads) =>
        println(s"Generating $numReads synthetic reads of length 101 bp")
        SyntheticReads.generate(101, numReads)
      case _ =>
        discount.getInputSequences(inFiles, method.addRCToMainData(discount))
    }
